...
module GamePage =
    ...
    let update msg model =
        match msg with
        | Tap pos ->
            match tapHandler model pos |> checkWin with
            | m, None when m.WithAi = true -> tapHandler m (getAiTap m.Field) |> checkWin
            | m, b -> m, b
    ...
    let view model =
        ContentPage(
            ...
            (Grid(coldefs = [ Star ], rowdefs = [ Stars 5.5; Stars 9.; Stars 5. ]) {
                ...
                (Grid(
                    coldefs = [ for _ in 0 .. model.Size - 1 -> Star ],
                    rowdefs = [ for _ in 0 .. model.Size - 1 -> Star ]
                ) {
