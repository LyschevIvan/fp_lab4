...
module App =
    ...
    let handleGoTo model (goTo: GoTo) =
        match goTo with
        | GamePage(n, withAi) ->
            { model with
                MainPageModel = None
                GamePageModel = Some(GamePage.init n withAi)
                EndGamePageModel = None }
        | EndGamePage(winner, size, withAi) ->
            { model with
                EndGamePageModel = Some(EndGamePage.init (winner, size, withAi))
                GamePageModel = None }
        | MainPage -> init ()
    ...
    let update msg (model: Model) =
        ...
        | GamePageMsg msg ->
            ...
            match goTo with
            | Some gt -> handleGoTo newModel gt
            | None -> newModel
        | MainPageMsg msg ->
